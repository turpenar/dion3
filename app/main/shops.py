...
class Shop(mixins.ReprMixin, mixins.DataFileMixin):
    ...
    def buy_item(self, number):
        if number is None and self._item_selected is None:
            ...
            return self.shop_result
        if number is None:
            self.update_character_output(character_output_text="Congratulations! You have purchased {}.".format(self._shop_items[self._item_selected].name))
            self.update_shop_item(shop_item=self._shop_items[self._item_selected])
            return self.shop_result
        else:
